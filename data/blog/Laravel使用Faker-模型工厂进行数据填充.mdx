---
title: 'Laravel使用模型工厂进行数据填充'
date: '2022-08-15'
tags: ['laravel']
draft: false
summary: '关于Laravel使用模型工厂进行数据填充的技术文章，包含详细的技术实现和最佳实践。'
---
...
`DatabaseSeeder` 使用 `call` 方法可以将数据填充拆分成多个文件，这样就不会使单个 `seeder` 变得非常大。只需简单传递要运行的 `seeder` 类名称即可
...
`db:seed` 命令将运行 `DatabaseSeeder` 类
...
echo $faker->VAT; //23456789### `Faker\Provider\Base 基本`
```
